const lines=["collect | verify | connect the dots","root@OSINTrahat:~# online","public noise → signal"];
...
const glyphs="01OSINT$#<>/\\";const font=14;let cols=Math.ceil(innerWidth/font);let drops=Array.from({length:cols},()=>Math.random()*-40);
...
const dHint=document.getElementById("discordHint");
if(dBtn){dBtn.addEventListener("click",async()=>{const v=dBtn.getAttribute("data-copy")||"rahatirtaza";try{await navigator.clipboard.writeText(v);dHint.textContent="Copied: @rahatirtaza";}catch(e){dHint.textContent="Username: @rahatirtaza";}setTimeout(()=>{dHint.textContent="Click to copy username";},1800);});}
